...
const AdminDashboardPage = () => {
  ...
  return isLoading ? (
    ...
    <div className="min-h-screen bg-muted/30 pb-20">
        ...
        <FadeIn>
          ...
          <div className="flex items-center gap-4 mb-8 overflow-x-auto pb-2 scrollbar-hide">
            <button onClick={() => setActiveTab('products')}
              ...
              <Package className="size-4 inline-block mr-2" /> Sản phẩm ({products.length})
            </button>
            <button onClick={() => setActiveTab('batches')}
              ...
              <Sprout className="size-4 inline-block mr-2" /> Lô hàng ({batches.length})
            </button>
            <button onClick={() => setActiveTab('traceability')}
              ...
              <ClipboardCheck className="size-4 inline-block mr-2" /> Truy xuất ({traces.filter(t => t.status === 'PENDING').length})
            </button>
            <button onClick={() => setActiveTab('orders')}
              ...
              <ShoppingBag className="size-4 inline-block mr-2" /> Đơn hàng ({orders.length})
            </button>
            {activeTab === 'products' && (
                    ...
                    {products.map((p) => (
                      <tr key={p.id} className="hover:bg-muted/20 transition-colors">
                        <td className="px-6 py-5 flex items-center gap-3">
                           {p.images?.[0] && <img src={p.images[0]} alt={p.name} className="size-10 rounded-lg object-cover" />}
                           <span className="font-bold">{p.name}</span>
                        </td>
                        <td className="px-6 py-5">{viVNFormatter.format(p.price)}đ</td>
                        <td className="px-6 py-5 text-center">
                          {p.isPublished ? <CheckCircle className="size-4 text-green-500 m-auto" /> : <XCircle className="size-4 text-muted-foreground m-auto" />}
                        </td>
                        <td className="px-6 py-5 text-right flex justify-end gap-2">
                          <button onClick={() => openEditForm(p)} className="p-2 hover:bg-muted rounded-lg"><Pencil className="size-4" /></button>
                          <button onClick={() => setDeleteConfirm(p.id)} className="p-2 hover:bg-destructive/10 text-destructive rounded-lg"><Trash2 className="size-4" /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="bg-card rounded-3xl border border-border shadow-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-border">
                    {traces.flatMap((t) => t.status === (activeTraceTab as any) ? [(
                      <tr key={t.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-6 py-5 text-right flex justify-end gap-2">
                          {t.status === 'PENDING' ? (
                            <>
                              <button onClick={() => updateTraceStatus(t.id, 'APPROVED')} className="p-2 text-green-600 hover:bg-green-500/10 rounded-lg"><CheckCircle className="size-4" /></button>
                              <button onClick={() => updateTraceStatus(t.id, 'REJECTED')} className="p-2 text-red-600 hover:bg-red-500/10 rounded-lg"><XCircle className="size-4" /></button>
                            </>
                          ) : (
                            <a href={`https://daklink.vn/trace/${t.batchNumber}`} target="_blank" rel="noopener noreferrer" className="p-2 text-muted-foreground hover:text-primary rounded-lg text-xs"><ExternalLink className="size-4" /></a>
                          )}
                        </td>
                      </tr>
                    )] : [])}
                  ...
                  <tbody className="divide-y divide-border">
                    {orders.map((o) => (
                      <tr key={o.id} className="hover:bg-muted/20 cursor-pointer" onClick={() => setSelectedOrder(o)}>
                        ...
                        <td className="px-6 py-5 text-right flex justify-end gap-2">
                          <button onClick={(e) => { e.stopPropagation(); setSelectedOrder(o); }} className="p-2 hover:bg-muted rounded-lg"><Eye className="size-4" /></button>
                          <button onClick={(e) => { e.stopPropagation(); setOrderToDelete(o.id); }} className="p-2 hover:bg-destructive/10 text-destructive rounded-lg"><Trash2 className="size-4" /></button>
                        </td>
                      </tr>
                    ))}
                    {orders.length === 0 && (
      ...
      <DashboardModals
        showForm={showForm} editingProduct={editingProduct} setShowForm={setShowForm} setEditingProduct={setEditingProduct} saveProduct={saveProduct} isSaving={isSaving} removeImage={removeImage} handleImageUpload={handleImageUpload}
        selectedOrder={selectedOrder} handleUpdateOrderStatus={handleUpdateOrderStatus} setSelectedOrder={setSelectedOrder}
        deleteConfirm={deleteConfirm} setDeleteConfirm={setDeleteConfirm} deleteProduct={deleteProduct}
        orderToDelete={orderToDelete} setOrderToDelete={setOrderToDelete} handleDeleteOrder={handleDeleteOrder}
      />
    </div>
  );
};
...
interface DashboardModalsProps {
  ...
  deleteProduct: any;
  orderToDelete: any;
  setOrderToDelete: any;
  handleDeleteOrder: any;
}

const DashboardModals = ({
  showForm, editingProduct, setShowForm, setEditingProduct, saveProduct, isSaving, removeImage, handleImageUpload,
  selectedOrder, handleUpdateOrderStatus, setSelectedOrder,
  deleteConfirm, setDeleteConfirm, deleteProduct,
  orderToDelete, setOrderToDelete, handleDeleteOrder
  ...
  <>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-3xl shadow-2xl border border-border w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <h2 className="text-xl font-semibold">{editingProduct.id ? 'Sửa sản phẩm' : 'Đăng sản phẩm'}</h2>
              <button onClick={() => { setShowForm(false); setEditingProduct(null); }} className="p-2 hover:bg-muted rounded-lg"><X className="size-5" /></button>
            </div>
            <div className="p-6 space-y-8">
              {/* Basic Info */}
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-primary uppercase border-b border-primary/10 pb-2">Thông tin cơ bản</h3>
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Tên sản phẩm *" value={editingProduct.name || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, name: v  }))} />
                  <FormField label="Slug *" value={editingProduct.slug || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, slug: v  }))} />
                </div>
                <div className="grid grid-cols-1 gap-4">
                  <FormField label="Mã hàng KiotViet (SKU)" value={editingProduct.sku || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, sku: v  }))} placeholder="Để trống nếu không đồng bộ KiotViet" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Giá mặc định (VNĐ)" value={String(editingProduct.price || 0)} onChange={(v) => setEditingProduct(prev => ({ ...prev!, price: parseInt(v) || 0  }))} type="number" />
                  <FormField label="Quy cách mặc định" value={editingProduct.size || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, size: v  }))} placeholder="vd: 1kg, 500g" />
                </div>
                {/* Options (Variants) */}
                <div className="bg-muted/20 p-4 rounded-xl border border-border space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Quy cách & Giá bán (Nhiều lựa chọn)</span>
                    <button type="button" onClick={() => {
                      const opts = [...(editingProduct.options || []), { size: '', price: 0 }];
                      setEditingProduct(prev => ({ ...prev!, options: opts  }));
                  ...
                  {editingProduct.options?.map((opt, idx) => (
                    <div key={opt.size || idx} className="flex items-end gap-3">
                      <div className="flex-1">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Quy cách</span>
                      ...
                      <div className="flex-1">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1">Giá (VNĐ)</span>
                ...
                <div className="grid grid-cols-2 gap-4 bg-muted/20 p-4 rounded-xl border border-border">
                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1.5">Tùy chọn truy xuất</span>
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        setEditingProduct(prev => ({ ...prev!, _traceType: val, batchNumber: newBatch  }));
                      }}
                      className="w-full bg-muted/50 border border-border focus:border-primary/50 rounded-xl py-2 px-3 text-foreground outline-none text-sm"
                    >
                  ...
                  {editingProduct._traceType === 'edenhub' && (
                    <FormField label="Link Edenhub" value={editingProduct.batchNumber || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, batchNumber: v  }))} placeholder="https://..." />
                  )}
                  {editingProduct._traceType === 'daklink' && (
                    <FormField label="Mã lô hàng Daklink" value={editingProduct.batchNumber || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, batchNumber: v  }))} placeholder="Nhập mã lô" />
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1.5">Danh mục</span>
                    <select value={editingProduct.category || 'trai-cay'} onChange={(e) => setEditingProduct(prev => ({ ...prev!, category: e.target.value  }))} className="w-full bg-muted/50 border border-border focus:border-primary/50 rounded-xl py-2 px-3 text-foreground outline-none text-sm">
                  ...
                  <FormField label="Nhãn (Badge)" value={editingProduct.badge || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, badge: v  }))} placeholder="vd: NEW, HOT, 20% OFF" />
                </div>
              </div>

              {/* Origin Section */}
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-primary uppercase border-b border-primary/10 pb-2">Nguồn gốc & Xuất xứ</h3>
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Quốc gia" value={editingProduct.origin?.country || 'Việt Nam'} onChange={(v) => setEditingProduct(prev => ({ ...prev!, origin: { ...prev!.origin!, country: v } }))} />
                  <FormField label="Vùng miền" value={editingProduct.origin?.region || 'Bình Thuận'} onChange={(v) => setEditingProduct(prev => ({ ...prev!, origin: { ...prev!.origin!, region: v } }))} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <FormField label="Nông trại / Cơ sở" value={editingProduct.origin?.farm || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, origin: { ...prev!.origin!, farm: v } }))} />
                  <FormField label="Người sản xuất" value={editingProduct.origin?.farmer || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, origin: { ...prev!.origin!, farmer: v } }))} />
                </div>
              </div>

              {/* Detailed Content */}
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-primary uppercase border-b border-primary/10 pb-2">Nội dung chi tiết</h3>
                <FormField label="Mô tả sản phẩm" value={editingProduct.description || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, description: v  }))} textarea />
                <FormField label="Câu chuyện thương hiệu" value={editingProduct.story || ''} onChange={(v) => setEditingProduct(prev => ({ ...prev!, story: v  }))} textarea />
              </div>
              
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-primary uppercase border-b border-primary/10 pb-2">Hình ảnh</h3>
                <div className="flex flex-wrap gap-2 pt-2">
                   {editingProduct.images?.map((img, i) => (
                     <div key={img} className="size-20 rounded-lg relative overflow-hidden">
                       <img src={img} alt="Ảnh sản phẩm" className="size-full object-cover" />
                       <button onClick={() => removeImage(i)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full"><X className="size-3" /></button>
                     </div>
                   ))}
                   <label className="size-20 border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer hover:bg-muted">
            ...
            <div className="p-6 border-t border-border flex justify-end gap-3">
              ...
              <button onClick={saveProduct} disabled={isSaving} className="px-8 py-2 bg-primary text-primary-foreground rounded-xl font-bold text-sm shadow-lg flex items-center gap-2">
                {isSaving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />} {editingProduct.id ? 'Cập nhật' : 'Đăng bán'}
              </button>
            </div>
          </div>
        </div>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-3xl shadow-2xl w-full max-w-2xl p-6">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2"><ShoppingBag className="size-5 text-primary" /> Chi tiết đơn hàng #{selectedOrder.id}</h2>
            <div className="space-y-6">
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <span className="text-[10px] font-bold text-muted-foreground uppercase mb-1 block">Trạng thái đơn</span>
                 ...
                 <div>
                   <span className="text-[10px] font-bold text-muted-foreground uppercase mb-1 block">Thanh toán</span>
               ...
               <div className="max-h-[30vh] overflow-y-auto space-y-3">
                 {selectedOrder.items?.map((item, i) => (
                   <div key={item.id || item.product_name || i} className="flex justify-between items-center text-sm p-3 bg-muted/20 rounded-xl">
                     <span className="font-semibold">{item.product_name} x {item.quantity}</span>
                     <span className="font-bold">{viVNFormatter.format(item.product_price * item.quantity)}đ</span>
                   </div>
                 ))}
               </div>
               ...
               <div className="pt-4 border-t border-border flex justify-between items-center font-bold text-xl">
                 <span>Tổng cộng:</span>
                 <span className="text-primary">{viVNFormatter.format(selectedOrder.total_amount)}đ</span>
               </div>
            </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
           <div className="bg-card rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
              <div className="size-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 className="size-8" /></div>
      ...
      {orderToDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
           <div className="bg-card rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
              <div className="size-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 className="size-8" /></div>
              <h3 className="text-lg font-semibold">Xác nhận xóa đơn hàng?</h3>
